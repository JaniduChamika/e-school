      ...
      <div class="table-responsive text-nowrap mb-4 scrollbar" style="min-height: 550px;max-height: 550px;">
            <table class="table table-hover expandable-table">
                  ...
                  <tbody class="table-border-bottom-0 ">
                        <?php
                        // search studets from student view 
                        $q = "SELECT * FROM `students`  ";
                        if (!empty($word)) {
                              // serach by student username, contact, email or full name
                              $q = $q . "WHERE  (`username` LIKE '" . $word . "%' OR `contact` LIKE '" . $word . "%' OR `email` LIKE '" . $word . "%' OR `fulname` LIKE '%" . $word . "%')";
                              if ($pen == "true" || $grade != 0) {
                                    $q = $q . "AND";
                              }
                        } else {
                        ...
                        // filter grade or pending status 
                        if ($grade != "0") {
                              $q = $q . " `gid`='" . $grade . "'";
                        }
                        if ($pen == "true" && $grade != 0) {
