...
if (isset($_POST["un"])) {
      ...
      if (empty($un)) {
            ...
            $result = DB::search("SELECT * FROM `user` WHERE (`username`='" . $un . "' OR `email`='".$un."') AND `password`='" . $pw . "' AND `user_type`='2'");
            if ($result->num_rows == 1) {
                  ...
                  if ($d["status_sid"] == 1) {
                        if (isset($_POST["vc"])) {
                              $vc = $_POST["vc"];
                              $result2 = DB::search("SELECT * FROM `user` WHERE (`username`='" . $un . "' OR `email`='".$un."') AND `password`='" . $pw . "' AND `user_type`='2' AND `v_code`='" . $vc . "'");
                              if ($result2->num_rows == 1) {
                                    ...
                                    DB::iud("UPDATE `user` SET `status_sid`='2',`v_code`='' WHERE (`username`='" . $un . "' OR `email`='".$un."') AND `user_type`='2'");
                                    $_SESSION["student"] = $d;
                                    echo "loginSucces";

                                    // search student is paid his enrollment fee for his latest grade start
                                    $stu = DB::search("SELECT * FROM `students` WHERE `uid`='" . $d["uid"] . "'");
                                    $stuD = $stu->fetch_assoc();
                                    $ispaid = DB::search("SELECT * FROM `student_payment` WHERE `user_uid`='" . $d["uid"] . "' AND `grade_gid`='" . $stuD["gid"] . "'");
                                    if ($ispaid->num_rows == 1) {

                                          $_SESSION["pay_status"] = "paid";
                                    } else {
                                          $istrial = DB::search("SELECT * FROM `trial_period` WHERE `user_uid`='" . $d["uid"] . "'");
                        ...
                        if ($d["tsv_status"] == 1) {
                              if (isset($_POST["vc"])) {
                                    ...
                                    if ($result2->num_rows == 1) {
                                          ...
                                          $_SESSION["student"] = $d;
                                          DB::iud("UPDATE `user` SET `v_code`='' WHERE (`username`='" . $un . "' OR `email`='".$un."') AND `user_type`='2'");
                                          echo "loginSucces";
                                    ...
                                    DB::iud("UPDATE `user` SET `v_code`='" . $vc . "' WHERE (`username`='" . $un . "' OR `email`='".$un."') AND `user_type`='2'");
