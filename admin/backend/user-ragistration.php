...
if (isset($_POST["userType"])) {
      ...
      if (empty($fname)) {
            ...
            $ishave = DB::search("SELECT * FROM `user` WHERE `username`='" . $uname . "' OR `email`='" . $email . "'");
            if ($ishave->num_rows >= 1) {
                  echo "Sorry! Username or email is Alrady registered";
            } else {

                  $lastid;
                  // start genarate password 
                  function password_generate($chars)
                  {
                        $data = '1234567890ABCDEFGHIJKLMNOPQRSTUVWXYZabcefghijklmnopqrstuvwxyz$@';
                        return substr(str_shuffle($data), 0, $chars);
                  }
                  $pw = password_generate(8);
                  // end genarate password 
                  $d = new DateTime();
                  ...
                  if ($userType == "admin") {
                        ...
                        // insert admin details to user table 
                        DB::iud("INSERT INTO `user` (`username`,`fname`,`lname`,`fulname`,`contact`,`email`,`password`,`dob`,`gender_gid`,`religion_rid`,`reg_date`
                  ,`user_type`,`status_sid`,`tsv_status`,`v_code`) 
                  VALUES ('" . $uname . "','" . $fname . "','" . $lname . "','" . $fullname . "','" . $contact . "','" . $email . "','" . $pw . "','" . $dob . "','" . $gender . "','" . $religion . "',
                  '" . $date . "','5','1','1','" . $vc . "')");

                        $lastid = DB::$dbms->insert_id;
                        // inset admin nic 
                        DB::iud("INSERT INTO `nic` (`user_uid`,`nic_no`) VALUES ('" . $lastid . "','" . $nic . "')");
                  } else if ($userType == "teacher") {
                        $UT = "3";
                        $sublen = $_POST["sublen"];
                        // insert teacher details to user table 
                        DB::iud("INSERT INTO `user` (`username`,`fname`,`lname`,`fulname`,`contact`,`email`,`password`,`dob`,`gender_gid`,`religion_rid`,`reg_date`
                              ,`user_type`,`status_sid`,`tsv_status`,`v_code`) 
                              VALUES ('" . $uname . "','" . $fname . "','" . $lname . "','" . $fullname . "','" . $contact . "','" . $email . "','" . $pw . "','" . $dob . "','" . $gender . "','" . $religion . "',
                              '" . $date . "','3','1','2','" . $vc . "')");
                        ...
                        for ($i = 0; $i < $gradeno->num_rows; $i++) {
                              ...
                              if ($grade == "true") {
                                    DB::iud("INSERT INTO `teacher_has_grade` (`user_uid`,`grade_gid`) VALUES ('" . $lastid . "','" . $gradeid . "')");
                              }
                        }
                        for ($i = 0; $i < $sublen; $i++) {
                              $sub = $_POST["sub" . $i];
                              ...
                              if ($sub == "true") {
                                    DB::iud("INSERT INTO `teacher_has_subject` (`user_uid`,`subject_id`) VALUES ('" . $lastid . "','" . $subid . "')");
                              }
                        }
                  } else if ($userType == "student") {
                        $UT = "2";
                        // insert student details to user table 
                        DB::iud("INSERT INTO `user` (`username`,`fname`,`lname`,`fulname`,`contact`,`email`,`password`,`dob`,`gender_gid`,`religion_rid`,`reg_date`
                           ,`user_type`,`status_sid`,`tsv_status`,`v_code`) 
                           VALUES ('" . $uname . "','" . $fname . "','" . $lname . "','" . $fullname . "','" . $contact . "','" . $email . "','" . $pw . "','" . $dob . "','" . $gender . "','" . $religion . "',
                           '" . $date . "','2','1','2','" . $vc . "')");
                        ...
                        $lastid = DB::$dbms->insert_id;
                        // insert student guardian details 
                        DB::iud("INSERT INTO `guardian_info` (`user_uid`,`name`,`contact_no`) VALUES ('" . $lastid . "','" . $gname . "','" . $gcontact . "')");

                        DB::iud("INSERT INTO `student_has_class` (`user_uid`,`class_id`) VALUES ('" . $lastid . "','" . $stuclass . "')");
                  } else if ($userType == "officer") {
                        $UT = "4";
                        // insert officer details to user table 
                        DB::iud("INSERT INTO `user` (`username`,`fname`,`lname`,`fulname`,`contact`,`email`,`password`,`dob`,`gender_gid`,`religion_rid`,`reg_date`
                          ,`user_type`,`status_sid`,`tsv_status`,`v_code`) 
                          VALUES ('" . $uname . "','" . $fname . "','" . $lname . "','" . $fullname . "','" . $contact . "','" . $email . "','" . $pw . "','" . $dob . "','" . $gender . "','" . $religion . "',
                          '" . $date . "','4','1','1','" . $vc . "')");
                  ...
                  DB::iud("INSERT INTO `address` (`address_line`,`user_uid`,`city_cid`) VALUES ('" . $address . "','" . $lastid . "','" . $city . "')");
                  ...
                  if (!in_array($imgType, $Allow_image_type)) {
                        ...
                        DB::iud("INSERT INTO `profile_image` (`user_uid`,`path`) VALUES ('" . $lastid . "','" . $imgName . "')");
                  }

                  // end image upload and rename 
