...
if (isset($_SESSION["student"]) && isset($_POST["fn"])) {

      $uid = $_SESSION["student"]["uid"];
      // get details send from client side 
      $fname = $_POST["fn"];
      $lname = $_POST["ln"];
      ...
      if (empty($fname)) {
            ...
            echo "emptyReligion";
      } else if (empty($guardname)) {
            echo "emptyGaurdianName";
      } else if (empty($guardcontact)) {
            echo "emptyemptyContact";
      } else if (preg_match("/07[0,1,2,4,5,6,7,8][0-9]+/", $guardcontact) == 0) {
            ...
            if ($ishave->num_rows >= 1) {
                  ...
                  // update user table acording to user id
                  ...
                  DB::iud("UPDATE `user` SET `fname`='" . $fname . "',`lname`='" . $lname . "',`fulname`='" . $fullname . "',
                      `contact`='" . $contact . "',`dob`='" . $dob . "',`gender_gid`='" . $gender . "',`religion_rid`='" . $religion . "'
                       WHERE `uid`='" . $uid . "' ");
                  // update gardian details 
                  DB::iud("UPDATE `guardian_info` SET `name`='" . $guardname . "',`contact_no`='" . $guardcontact . "' WHERE `user_uid`='" . $uid . "'");
                  // update address 
